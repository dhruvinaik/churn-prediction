...
page_bg = """
<style>
/* Main animated background (updated) */
[data-testid="stAppViewContainer"] {
    background: linear-gradient(120deg, #ffd3e0 0%, #ffe6c9 25%, #cfe9ff 50%, #f0f7ff 75%, #fbeefc 100%);
    background-size: 300% 300%;
    animation: mainGradient 18s ease infinite;
}

/* Slight soft overlay on content for readability */
...
/* Sidebar animated gradient (updated) */
[data-testid="stSidebar"] {
    background: linear-gradient(135deg, #7b2ff7 0%, #2bd2ff 50%, #6dd3b0 100%);
    background-size: 400% 400%;
    animation: sidebarGradient 9s ease infinite;
    color: white;
    backdrop-filter: blur(6px);
    border-right: 2px solid rgba(255,255,255,0.12);
}

/* Ensure sidebar text and labels are readable */
...
/* Heading style kept as original */
...
day = st.sidebar.selectbox("📅 Day of Week", weekday_order)
