...
def generate_hotspots_from_churn(city, churn_prob, spread_km=12.0, seed=None):
    """
    Create a set of lat/lon/weight points around a city center.
    churn_prob is 0..1, weight scales with churn_prob and a synthetic user count.
    """
    rng = np.random.default_rng(seed)
    center = CITY_COORDS.get(city, (20.0, 77.0))
    lat0, lon0 = center
    # decide number of hotspots based on churn intensity
    num_spots = clamp(int(churn_prob * 20) + 4, 3, 30)
    pts = []
    # synthetic scale: base_users approximated by churn intensity
    ...
    for _ in range(num_spots):
        ...
        lat = lat0 + dlat
        lon = lon0 + dlon
        # weight: combine base_users with random factor and churn_prob
...
def create_heatmap_html_churn(city, hotspots, events=None, map_zoom=12, tiles="CartoDB dark_matter"):
    ...
    # optionally draw events as orange markers
    if events:
        for ev in events:
...
def load_dataset():
    ...
    if p.exists():
        try:
            ...
            pass
    # fallback empty df with columns used in this project
...
# ----------------------------
# Encoding helper (match encoders saved)
# ----------------------------
def safe_encode(encoders, col, value):
    ...
    if enc is None:
        return value
    try:
        # try to transform direct match; if not present, attempt case-insensitive match
...
# ----------------------------
# Streamlit UI (style from original proj.py)
# ----------------------------
...
st.title("🚖 Auto & Taxi Usage — Churn Prediction (Advanced)")
...
if predict_clicked:
    ...
    # UI: show churn probability & risk cards similar to proj.py metrics
    ...
    # Heatmap (use churn probability to generate hotspots around chosen city)
    try:
        hotspots = generate_hotspots_from_churn(raw_inputs["location"], proba, spread_km=10.0, seed=42)
        events = []  # keep compatibility with original create_heatmap_html; we have none
    ...
    # Append to history (store a tuple similar to original for compatibility)
...
with tab1:
    ...
    if "last_trip_days_ago" in df.columns and "churn" in df.columns and not df.empty:
        # show mean churn by binned last_trip_days_ago
        df_plot = df.copy()
...
# --- Tab 2: Dynamic from predictions ---
with tab2:
    ...
    if st.session_state.history:
        ...
        with col1:
            st.markdown("### 🌆 Average Predicted Churn by City")
            city_data = hist_df.groupby("City")["ChurnProb"].mean().sort_values(ascending=False)
            st.bar_chart(city_data)

        with col2:
            st.markdown("### 🚘 Predicted Churn by Trip Type")
            trip_type_data = hist_df.groupby("Trip Type")["ChurnProb"].mean().sort_values(ascending=False)
            st.bar_chart(trip_type_data)

        st.markdown("### 📅 Predicted Churn by Day of Week")
        day_data = hist_df.groupby("Day")["ChurnProb"].mean().sort_values(ascending=False)
        ...
        for _, row in hist_df.iterrows():
            ...
            if loc in CITY_COORDS:
                ...
                # weight by churn prob (scale up for visibility)
